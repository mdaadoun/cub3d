...
static void	fs_display_vision(t_cub *cub)
{
	...
	fov_left.x2 = (((cub->player->map_x + cub->player->grid_x) - sin(cub->player->angle - cub->player->fov / 2)) * (GRID));
	fov_left.y2 = (((cub->player->map_y + cub->player->grid_y) + cos(cub->player->angle - cub->player->fov / 2)) * (GRID));
	fov_right.x1 = (cub->player->map_x + cub->player->grid_x) * GRID;
	fov_right.y1 = (cub->player->map_y + cub->player->grid_y) * GRID;
	fov_right.x2 = (((cub->player->map_x + cub->player->grid_x) - sin(cub->player->angle + cub->player->fov / 2)) * (GRID));
	fov_right.y2 = (((cub->player->map_y + cub->player->grid_y) + cos(cub->player->angle + cub->player->fov / 2)) * (GRID));
